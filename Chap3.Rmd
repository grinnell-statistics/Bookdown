...
```{r fig3.2, echo=FALSE, out.width="100%", fig.align="center", fig.cap="Common shapes of residual plots. Ideally, residual plots should look like a randomly scattered set of dropped coins, as seen in the oval-shaped plot. If a pattern exists, it is usually best to try other regression models."}
...
```{r fig3.3, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.3** Residual versus fit plots using Price and TPrice (the log10 transformation), as responses. The residual plot with Price as the response has a much stronger wedge-shaped pattern than the one with TPrice."}
...
```{r fig3.4, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.4** Residual versus order plots using TPrice as the response. The first graph uses Mileage as the explanatory variable, and the second graph uses Mileage, Cyl, Doors, Cruise, Sound, and Leather as explanatory variables."}
...
```{r fig3.5, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.5** Residual versus fit plots using Price and TPrice as the response. The circled observations in the plot using Price are no longer clear outliers in the plot using TPrice."}
...
```{r fig3.6, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.6** Normal probability plot and histogram of residuals from the model using TPrice as the response and Mileage, Cyl, Doors, Cruise, Sound, and Leather as the explanatory variables."}
...
```{r tab3.1, echo=FALSE, results='asis'}
df3.1 <- data.frame(
  Assumption = c(
    "Normality",
    "Zero mean",
...
cat(
  kable(
    ...
    caption = "Table 3.1 Plots that can be used to evaluate model assumptions about the residuals.",
    align = c("l", "l")
  ) %>%
...
```{r fig3.7, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.7** Scatterplot showing a clear association between Liter and Cyl."}
...
```{r fig3.8, echo=FALSE, out.width="100%", fig.align="center", fig.cap="**Figure 3.8** Residual versus order plots show that incorporating the indicator variables into the regression model improves the random behavior and reduces the sizes of the residuals."}
...
```{r df_1-plot, echo=FALSE, fig.width=6, fig.height=4, message=FALSE, fig.cap="Scatterplot and regression line for Chevrolet Cavalier Sedans: Price = 15,244 - 0.111(Mileage)."}
# read full file
#df <- read.csv("C:/Users/kuipers/Dropbox/Kuiper Sklar Public/Clean csv data/C3Cars.csv", stringsAsFactors = FALSE)

data("C10Cars")
df<-C10Cars

# filter to only Chevrolet Cavalier Sedans
df_1 <- df %>% filter(Make  == "Chevrolet", Model == "Cavalier", Type == "Sedan")

# plot
ggplot(df_1, aes(x = Mileage, y = Price)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Scatterplot with Regression Line for\nChevrolet Cavalier Sedans",
    x     = "Mileage",
    y     = "Price"
  ) +
  ...
  theme(
    ...
    plot.margin = margin(t = 20, r = 45, b = 20, l = 15, unit = "pt")
  )
```
...
```{r df_2-plot, echo=FALSE, fig.width=6, fig.height=3.7, message=FALSE, fig.pos='H', fig.cap="Scatterplot and least squares regression line: Price = 15,349 - 0.20(Mileage) + 3443(Cyl). For each cylinder size, an increase of one mile is expected to reduce price by $0.20."}
# read full file
df_2 <- read.csv("https://raw.githubusercontent.com/grinnell-statistics/STA310data/refs/heads/master/Cars2.csv", stringsAsFactors = FALSE)

# turn Cyl into a factor, for categ vars
df_2$Cyl <- factor(df_2$Cyl)

m <- lm(Price ~ Mileage + Cyl, data = df_2)
b0 <- coef(m)[1]
b1 <- coef(m)[2]
b2 <- coef(m)[3]

# plot
ggplot(df_2, aes(x = Mileage, y = Price, color = Cyl)) +
  geom_point() +
  geom_abline(intercept = b0, slope = b1, color = "black") +
  geom_abline(intercept = b0 + b2, slope = b1, color = "#00BFC4") +
  labs(
...
```{r df_3-plot, echo=FALSE, fig.width=6, fig.height=3.7, message=FALSE, fig.cap="Scatterplot and and least squares regression line: Price = 4533 + 0.340(Mileage) + 5431(Cyl) + 0.0995(MileCyl). If the interaction term (MileCyl) is important, we expect to have regression lines that are not parallel."}
# read full file
df_3 <- read.csv("https://raw.githubusercontent.com/grinnell-statistics/STA310data/refs/heads/master/Cars2.csv", stringsAsFactors = FALSE)

# turn Cyl into a factor, for categ vars
df_3$Cyl <- factor(df_3$Cyl)

# plot
ggplot(df_3, aes(x = Mileage, y = Price, color = Cyl)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Scatterplot of Price vs Mileage",
...
```{r df_4-plot, echo=FALSE, fig.width=6, fig.height=3.5, message=FALSE, fig.cap="Scatterplot and regression lines: For the Cadillac XLR-V8, the regression line is Price = 71,997 - 0.4827(Mileage). This is a much steeper line than the regression line for all other cars: Price = 23,894 - 0.1549(Mileage)."}
# read full file
df_4 <- read.csv("https://raw.githubusercontent.com/grinnell-statistics/STA310data/refs/heads/master/Cars2.csv", stringsAsFactors = FALSE)

# df_4$Model <- factor(df_4$Model)
# model.matrix(~ Model, data = df_4)
# one-hot encoding for "XLR-V8"
df_4$Model2 <- factor(
  ifelse(df_4$Model == "XLR-V8", "XLR-V8", "Other"),
  levels = c("XLR-V8", "Other")
)
mm <- model.matrix(~ Model2, data = df_4)

# plot
ggplot(df_4, aes(x = Mileage, y = Price, color = Model2)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  scale_colour_manual(
    name = "Model",
    values = c("XLR-V8" = "#00BFC4",
               "Other"  = "black")
  ) +
  labs(
    title = "Scatterplot of Retail Price vs Mileage",
    x     = "Mileage",
    y     = "Retail Price"
  ) +
  theme_minimal(base_family = "serif") +
  theme(
    ...
    # text
    plot.title = element_text(size = 16, hjust = 0.5),
    plot.margin = margin(t = 20, r = 10, b = 10, l = 15, unit = "pt")
  )
```
...
[[[The table is not displaying. But it works when I knit on separate pdf]]]
```{r tab3.3, echo=FALSE, results='asis'}
df3.3 <- data.frame(
  Statistic = c(
    "$R^2 = 1 - \\dfrac{\\sum_{i=1}^n (y_i - \\hat{y}_i)^2}{\\sum_{i=1}^n (y_i - \\overline{y})^2}$",
...
cat(
  kable(
    ...
    caption = "Table 3.3 Variable selection criteria.",
    align = c("l", "l")
  ) %>%
    kable_styling(latex_options = "hold_position")
)
...
```{r tab3.4, echo=FALSE, results='asis'}
...
cat(
  kable(
    ...
    caption = "Table 3.4 The extra sum of squares $F$-statistic is the ratio of the mean square for the extra $k-p$ terms to the $\\text{MSE}_{\\text{Full}}$ with $k-p$ and $n-k$ degrees of freedom.",
    align = c("l", "c", "c", "c", "c")
  ) %>%
    kable_styling(latex_options = "hold_position")
)
