...
```{r fig4.1, warning=FALSE, echo=FALSE, fig.width=6, fig.height=4, message=FALSE, fig.cap="Individual value plot of the PopRate (100 $\times$ count of popped kernels/total kernels) for each Brand and cooking Time factor-level combination. Points have jittering (small fluctuations) so that all values are visible."}
library(readr)
library(dplyr)
library(ggplot2)
...
ggplot(df, aes(x = Time, y = PopRate)) +
  geom_jitter(
    width = 0.1,
    height = 0,
    size  = 2,
